...
class PerturbationBatchSampler(Sampler):
    ...
    def __init__(
        ...
        self._num_batches = len(self.batches)  # Cache the number of batches for __len__
        self.tot_num = tot_num
        end_time = time.time()
        logger.info(
            f"Sampler created with {self._num_batches} batches in {end_time - start_time:.2f} seconds."
        )
    ...
    def __iter__(self) -> Iterator[list[int]]:
        ...
        if not self.distributed:
            self.batches = self._create_batches()
            # Update cached length (should be same, but update for consistency)
            self._num_batches = len(self.batches)
        yield from self.batches

    def __len__(self) -> int:
        # Return cached length for consistent behavior
        # This is critical for PyTorch Lightning to show progress bar correctly
        # Ensure we always have a valid length
        if not hasattr(self, '_num_batches') or self._num_batches is None:
            # If batches don't exist yet, create them
