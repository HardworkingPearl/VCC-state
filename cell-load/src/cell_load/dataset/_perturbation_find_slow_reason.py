...
class PerturbationDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Pre-decode cell_type_categories if they're bytes (avoid repeated decoding in __getitem__)
        if len(self.cell_type_categories) > 0 and isinstance(self.cell_type_categories[0], bytes):
            self.cell_type_categories = np.array([ct.decode("utf-8") if isinstance(ct, bytes) else str(ct) 
                                                  for ct in self.cell_type_categories])
        ...
        # Cache expensive HDF5 reads for performance (especially important for h5ad files)
        self._x_attrs = dict(self.h5_file["X"].attrs)  # Cache X attributes
        self._x_is_csr = self._x_attrs.get("encoding-type") == "csr_matrix"
        if self._x_is_csr:
            # Cache dataset references for CSR matrices (avoid repeated lookups)
        ...
        # Pre-compute alignment indices if var_index exists (avoid expensive np.intersect1d in hot path)
        self._alignment_indices = None
        if self._var_index is not None:
            try:
                # Compute intersection once and cache the indices
                _, idx_base, idx_var = np.intersect1d(gene_base_index, self._var_index, return_indices=True)
                self._alignment_indices = (idx_base, idx_var)
            except Exception:
                # If alignment fails, will fall back to calling align_vector_bytes
                self._alignment_indices = None
    ...
    def __getitem__(self, idx: int):
        ...
        # Cell type info (cell_type_categories already decoded during init)
    ...
    def fetch_gene_expression(self, idx: int) -> torch.Tensor:
        ...
        if self._x_is_csr:
            # Use cached dataset references (much faster for h5ad files)
        ...
        if len(data) != 18080:
            # TEST: Replace with zeros to check if alignment is still slow
            data = torch.zeros(gene_base_index.shape[0], dtype=data.dtype, device=data.device)
            # Original code (commented out for testing):
            # # Use pre-computed alignment indices if available (much faster than np.intersect1d)
            ...
            #     data = aligned
            # else:
            #     # Fallback to original function if indices not pre-computed
    ...
    def fetch_obsm_expression(self, idx: int, key: str) -> torch.Tensor:
        ...
        """
        # Use cached dataset reference if available (most common case)
