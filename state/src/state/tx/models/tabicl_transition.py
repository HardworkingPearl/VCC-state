...
class TabICLModel(PerturbationModel):
# class StateTransitionPerturbationModel(PerturbationModel):
    ...
    def _build_networks(self, lora_cfg=None):
        ...
        self.tabicl_mdl._load_model()
        self.tabicl_mdl.model_ = self.tabicl_mdl.model_.to('cuda')
        self.tabicl_context_X = nn.Parameter(torch.randn(16, self.cell_sentence_len, self.hidden_dim)).to('cuda')
        self.tabicl_context_Y = nn.Parameter(torch.randn(16, self.cell_sentence_len, 18080)).to('cuda')  # pert_cell_emb=18080
    ...
    def forward(self, batch: dict, padded=True) -> torch.Tensor:
        ...
        if self.hparams.get("mask_attn", False):  # skip
            ...
            B, T, H = X.shape
            R = torch.cat([self.tabicl_context_X, X], dim=1)
            train_size = self.tabicl_context_Y.shape[1]  # from  https://github.com/soda-inria/tabicl/blob/main/src/tabicl/model/learning.py#L14
            
            R = self.tabicl_mdl.model_.row_interactor(self.tabicl_mdl.model_.col_embedder(R, d=self.hidden_dim, train_size=train_size), d=self.hidden_dim)
            # tabicl_output = self.tabicl_mdl.model_.icl_predictor(representations, )
            # R[:, :train_size] = R[:, :train_size] + self.pert_y_encoder(self.tabicl_context_Y)  # self.basal_encoder(self.tabicl_context_Y)  # self.tabicl_mdl.model_.icl_predictor.y_encoder(self.tabicl_context_Y)
            enc = self.pert_y_encoder(self.tabicl_context_Y)            # computed with grad
            left  = R[:, :train_size] + enc                             # out-of-place add
            right = R[:, train_size:]                                   # untouched part
            R = torch.cat([left, right], dim=1)                         # new R, no inplace
            src = self.tabicl_mdl.model_.icl_predictor.tf_icl(R, attn_mask=train_size)[:, train_size:]
            # self.tabicl_mdl.model_.column_interactor: 398,216
            # self.tabicl_mdl.model_.col_embedder: 877,824
            # self.tabicl_mdl.model_.icl_predictor.y_encoder: 5,632
            # self.tabicl_mdl.model_.icl_predictor.tf_icl: 25,233,408
        ...
        if self.confidence_token is not None:   # skip
            ...
            res_pred = src # transformer_output
