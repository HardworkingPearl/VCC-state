...
class CausalPFNModel(PerturbationModel):
    ...
    def forward(self, batch: dict, padded=True) -> torch.Tensor:
        ...
        """
        # Handle None, crop, or pad to size 5120 - Optimized for performance
        target_size = 5120
        pert_emb_list = batch["pert_emb"]
        batch_size = len(pert_emb_list)
        
        # Pre-allocate output tensor on GPU
        processed_embs = torch.zeros(batch_size, target_size, dtype=torch.float32, device='cuda')
        
        for i, emb in enumerate(pert_emb_list):
            if emb is None:
                # Already zeros, skip
                continue
            else:
                # Flatten to 1D and move to GPU
                emb_flat = emb.flatten().to('cuda')
                flat_size = emb_flat.numel()
                
                if flat_size > target_size:
                    # Crop: take first target_size elements
                    processed_embs[i, :] = emb_flat[:target_size]
                elif flat_size < target_size:
                    # Pad: copy to beginning, rest is already zero
                    processed_embs[i, :flat_size] = emb_flat
                else:
                    # Already correct size
                    processed_embs[i, :] = emb_flat
        
        batch["pert_emb"] = processed_embs
        # target_size = 5120
