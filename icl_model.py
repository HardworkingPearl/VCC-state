...
class InContextModel(nn.Module):
    ...
    def _get_y0_y1_shift_scale(
        ...
        if len(y_context.shape)==2:
            ...
            y1_mean = (y_context * treated_mask[..., None]).sum(dim=1, keepdim=True) / treated_counts
            y0_mean = (y_context * control_mask[..., None]).sum(dim=1, keepdim=True) / control_counts
            # make modifications here! QH
            y1_std = (
                torch.sqrt(((y_context - y1_mean) ** 2 * treated_mask[..., None]).sum(dim=1, keepdim=True) / treated_counts) + 1e-20
            )
            y0_std = (
                torch.sqrt(((y_context - y0_mean) ** 2 * control_mask[..., None]).sum(dim=1, keepdim=True) / control_counts) + 1e-20
            )
    ...
    def predict_cepo(
        ...
        #     t_context[..., None] == 1, (X_context - x1_shift) / x1_scale, (X_context - x0_shift) / x0_scale
        # )
        # X_query = torch.where(
        #     t_query[..., None] == 1, (X_query - x1_shift) / x1_scale, (X_query - x0_shift) / x0_scale
        # )
        # concatenate ..., difference is only normalization for ctrl and treatment?
        x_and_t_context = torch.cat(
            [
                t_context.unsqueeze(-1),
                X_context,
